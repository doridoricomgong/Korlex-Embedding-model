...
import re
from KorLex_API.KorLex_api.krx_api import *
...
def build_neighbors_map(vocab_dict, full_graph=None, krx_json_api=None):
    global neighbors_dict
    f = open('/home/ai/다운로드/benPark/path2vec/Korean_Korlex_dataset/korea_train/voca/neighbor_list.json')
    data = dict(json.load(f))
    f.close()
    f = open('/home/ai/다운로드/benPark/path2vec/Korean_Korlex_dataset/korea_train/voca/same_soff_list.json')
    soff = dict(json.load(f))
    f.close()
    neighbors_dict = copy.deepcopy({int(k): l for k, l in data.items()})
    for idx, value in neighbors_dict.items():
        for num in soff[str(idx)]:
            if num == idx:
                continue
            if num in value:
                continue
            neighbors_dict[idx].append(num)

    '''global neighbors_dict, out_of_voc
    neighbor_nodes = []

    for vocab, index in vocab_dict.items():
        if index % 1000 == 0:
            print('progress: ', index, '/', len(vocab_dict))
        if out_of_form(vocab) is None:
            out_of_voc.append([vocab, index])
            continue
        if vocab == 'UNK':
            continue
        ...
        if full_graph:
            ...
            # changed for Korlex
            try:
                synset = krx_json_api.synset(vocab)
            except:
                out_of_voc.append(vocab)

            hypernyms = synset.hypernyms()
            hyponyms = synset.hyponyms()
            for hypernym in hypernyms:
                if vocab_dict.get(str(hypernym), -1) == -1:
                    out_of_voc.append(hypernym)
                else:
                    neighbor_nodes.append(vocab_dict[str(hypernym)])
            for hyponym in hyponyms:
                if vocab_dict.get(str(hyponym), -1) == -1:
                    out_of_voc.append(hyponym)
                else:
                    neighbor_nodes.append(vocab_dict[str(hyponym)])
    '''
    '''
...
def batch_generator_2(pairs, vocabulary, vocab_size, nsize, batch_size):
    ...
    samples_per_pair = 2 + 2 * nsize  # How
    # y training instances we get from each pair
    # How many samples will be there in each batch?
    samples_per_batch = samples_per_pair * batch_size
    ...
    for pair in pairs:
        ...
        for i in range(samples_per_pair):
            #print('batchs', len(batch[0][0]), len(batch[0][0][0]))
            batch[0][0][inst_counter] = neg_samples[0][i][0]
            batch[0][1][inst_counter] = neg_samples[0][i][1]
...
def get_negative_samples(current_word_index, context_word_index, vocab_size, nsize):
    ...
    if pre_curr == 0 :
        for i in range(len(neg_samples[0])):
            if neg_samples[0][i][0] == 1 and neg_samples[1][i] == 1:
                neg_samples[0][i][0] = 0
    if pre_cont == 0:
        for i in range(len(neg_samples[0])):
            if neg_samples[0][i][1] == 1 and neg_samples[1][i] == 1:
                neg_samples[0][i][1] = 0
    return neg_samples
...
#ben park / check if form is not 'word.pos.01'
def out_of_form(string):
    pattern = re.compile(r'.*[.]{1}[a-z]{1}[.]{1}[0-9]{2}')
...
def out_of_vocab():
    global out_of_voc
    f = open("./OOV.txt", "a")
    f.write('OOV num:' + str(len(out_of_voc)) + '\n')
    for voc, idx in enumerate(out_of_voc):
        f.write(voc + '  '+ str(idx) + '\n')
    f.close()
