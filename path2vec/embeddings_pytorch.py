...
def custom_loss(y_pred, y_true, reg_1_output, reg_2_output, use_neighbors, beta=0.01, gamma=0.01):
    if use_neighbors:
        alpha = 1 - (beta+gamma)
        m_loss = alpha * F.mse_loss(y_pred, y_true, reduction='mean')
        
        m_loss -= beta * reg_1_output
        m_loss -= gamma * reg_2_output
    else:
        m_loss = F.mse_loss(y_pred, y_true, reduction='mean')
...
def run(trainfile, vocab_file, embedding_dimension, batch_size, learn_rate, neighbors_count, negative, run_name, 
        l1_factor, beta, gamma, fix_seeds, epochs, use_neighbors, regularize, krx_json_api):
    if fix_seeds:
    ...
    helpers.build_neighbors_map(vocab_dict, full_graph=False, krx_json_api= krx_json_api)
    ...
    for epoch in range(epochs):
        ...
        for batch in batchGenerator:
            ...
            #print('target: ', np.array(targets).shape)
            target_tensor = torch.from_numpy(targets).float()


            #input [2,800, vocab_size(73388)]
            #input_var = torch.Tensor([inputs[0], inputs[1]]).long()
            input_var = torch.LongTensor(np.array(inputs))
            ...
            model.zero_grad()
            #print(vocab_size, torch.max(input_var), max(vocab_dict.values()))
            #do the forward pass
            ...
            if use_neighbors:
                ...
                # if input vector is empty == no positive_samples
                if len(input_var[0]) == 0:
                    continue
    ...
    try:
        train_name = trainfile.split('/')[-1].split('.')[0] + '_embeddings_vsize' + str(embedding_dimension) +'_bsize' + str(batch_size) \
                     + '_lr' + str(learn_rate).split('.')[-1]+'_nn-'+str(args.use_neighbors)+str(args.neighbor_count)+\
                     '_reg-'+str(args.regularize)
    except:
        train_name = 'temp' + datetime.time
